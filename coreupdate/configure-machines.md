...
Configuring new or existing Flatcar Container Linux machines to communicate with a [CoreUpdate](https://coreos.com/products/coreupdate) instance is a simple change to a configuration file.
...
By default, your installation has a single application, Flatcar Container Linux, with the identifier `e96281a6-d1af-4bde-9a0a-97b76e56dc57`. This ID is universal and all Flatcar Container Linux machines are configured to use it. Within the Flatcar Container Linux application, there are several application groups which have been created to match CoreOS Container Linux channels with the identifiers `alpha`, `beta`, and `stable`.
...
To place a Flatcar Container Linux machine in one of these groups, you must configure the update settings via a Container Linux Config or a file on disk.
...
`COREOS_RELEASE_APPID`: the Flatcar Container Linux app ID, `e96281a6-d1af-4bde-9a0a-97b76e56dc57`

`COREOS_RELEASE_VERSION`: defaults to the version of Flatcar Container Linux you're running
