...
Read [concepts](docs/lokomotive/architecture/concepts.md) to learn about Terraform, modules, and organizing resources. Change to your infrastructure repository (e.g. `infra`).
...
AWS allows additional groups of workers to be defined and joined to a cluster. For example, add worker pools of instances with different types, disk sizes, Container Linux channels, or preemptibility modes.

To define a worker pool, we will use the internal Terraform Module: `aws/container-linux/kubernetes/workers`.
